...
export const getAllCategories: () => Category[] = cache(() => {
  console.log("Executing getAllCategories (will run once per request if called multiple times)");
  return categories;
});
...
export const getCategory: (categoryId: string) => Category | undefined = cache(
  (categoryId: string) => {
    console.log(`Executing getCategory for ID: ${categoryId} (will run once per ID per request)`);
...
export const getCategoryQuestions: (categoryId: string) => Question[] = cache(
  (categoryId: string) => {
    console.log(`Executing getCategoryQuestions for ID: ${categoryId} (will run once per ID per request)`);
 ...
 */
export const getQuestionById: (questionId: string) => Question | undefined = cache(
  (questionId: string) => {
    console.log(`Executing getQuestionById for ID: ${questionId} (will run once per ID per request)`);
